...
#pragma region spMeshResourceDescriptor
  ...
  void spMeshResourceDescriptor::Clear()
  {
    m_uiNumLOD = 0;
    m_LODs.Clear();
    m_LODs.Reserve(SP_RPI_MAX_LOD_COUNT);
  }

  const spMesh& spMeshResourceDescriptor::GetLOD(ezUInt32 uiLodIndex) const
  {
    EZ_ASSERT_DEV(uiLodIndex < SP_RPI_MAX_LOD_COUNT, "Invalid LOD index {0}. Value is greater than the maximum number of LODs.", uiLodIndex);
