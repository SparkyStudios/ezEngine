...
namespace RHI
{
  ...
  /// \brief A handle for every resource created by the graphics device.
  class spResourceHandle
  {
    EZ_DECLARE_HANDLE_TYPE(spResourceHandle, spResourceHandleId);
  };

  /// \brief A handle for every context created by the graphics device.
  ...
  struct spViewport : ezHashableStruct<spViewport>
  {
    ...
    [[nodiscard]] EZ_ALWAYS_INLINE float GetAspectRatio() const { return static_cast<float>(m_uiWidth) / m_uiHeight; }
  ...
  struct spGraphicsApi
  {
    ...
    enum Enum : StorageType
    {
      ...
      /// \brief Direct3D 11 API (Windows, UWP, Xbox 360)
      Direct3D11,
  ...
  struct spGraphicsApiVersion
  {
    ...
    [[nodiscard]] EZ_ALWAYS_INLINE bool operator==(const spGraphicsApiVersion& rhs) const
    {
