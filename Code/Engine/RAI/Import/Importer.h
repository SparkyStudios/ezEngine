...
namespace RAI
{
  ...
  public:
    ...
    /// \brief Imports the file at the given path and convert it to an asset.
    /// \param sAssetPath The path to the raw asset file to import.
    /// \param [out] out_pAsset The output asset array.
    EZ_ALWAYS_INLINE ezResult Import(ezStringView sAssetPath, ezArrayPtr<TAsset> out_pAsset) { return Import(sAssetPath, out_pAsset.GetPtr(), out_pAsset.GetCount()); }
