...
/// \brief A stream reader that will decompress data that was stored using the ezCompressedStreamWriterBrotliG.
///
/// The reader takes another reader as its source for the compressed data (e.g. a file or a memory stream).
class EZ_FOUNDATION_DLL ezCompressedStreamReaderBrotliG : public ezStreamReader
{
public:
  ezCompressedStreamReaderBrotliG(); // [tested]

  /// \brief Takes an input stream as the source from which to read the compressed data.
  ezCompressedStreamReaderBrotliG(ezStreamReader* pInputStream); // [tested]
  ...
  ezUInt64 ReadBytes(void* pReadBuffer, ezUInt64 uiBytesToRead) override; // [tested]

private:
  ezDynamicArray<ezUInt8> m_DecompressedData;
  ezUInt64 m_uiCursor{0};
...
class EZ_FOUNDATION_DLL ezCompressedStreamWriterBrotliG : public ezStreamWriter
{
public:
  ezCompressedStreamWriterBrotliG();

  /// \brief The constructor takes another stream writer to pass the output into.
  ezCompressedStreamWriterBrotliG(ezStreamWriter* pOutputStream); // [tested]

  /// \brief Calls FinishCompressedStream() internally.
  ~ezCompressedStreamWriterBrotliG(); // [tested]

  /// \brief Configures to which other ezStreamWriter the compressed data should be passed along.
  ///
  /// Also configures how much data (in KB) should be cached internally before passing it to the compressor.
  /// Adjusting this cache size is only of interest, if the compressed output needs to be consumed as quickly as possible
  ...
  void SetOutputStream(ezStreamWriter* pOutputStream, ezUInt32 uiPageSizeKB = 64); // [tested]
  ...
  EZ_NODISCARD EZ_ALWAYS_INLINE ezUInt64 GetUncompressedSize() const { return m_uiUncompressedSize; } // [tested]
  ...
  EZ_NODISCARD EZ_ALWAYS_INLINE ezUInt64 GetCompressedSize() const { return m_uiCompressedSize; } // [tested]
  ...
  EZ_NODISCARD EZ_ALWAYS_INLINE ezUInt64 GetWrittenBytes() const { return m_uiWrittenBytes; } // [tested]
...
private:
  ...
  ezUInt64 m_uiPageSize{65536};

  ezStreamWriter* m_pOutputStream = nullptr;

  ezDynamicArray<ezUInt8> m_CompressedCache;
  ezDynamicArray<ezUInt8> m_UncompressedCache;
};
