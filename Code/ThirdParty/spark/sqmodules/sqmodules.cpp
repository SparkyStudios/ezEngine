...
#include <squirrel/sqstddatetime.h>
#include <squirrel/sqstdaux.h>
#include <squirrel/sqdirect.h>
...
void SqModules::registerNativeModule(const char *name, RegFunc reg_func)
{
  HSQOBJECT hModule;
  sq_newtable(sqvm);
...
void SqModules::setupMainModule()
{
  SQInteger prevTop = sq_gettop(sqvm);
  (void)prevTop;
  ...
  HSQOBJECT hBindings;
  sq_getstackobj(sqvm, -1, &hBindings);
  SqObjPtr bindingsTbl(sqvm, hBindings); // add ref
  SqObjPtr stateStorage = setupStateStorage("__main__");

  assert(sq_gettop(sqvm) == prevTop+1); // bindings table

  sq_pushstring(sqvm, "persist", 7);
  sq_pushobject(sqvm, stateStorage.o);
  sq_newclosure(sqvm, persist_state, 1);
  sq_setparamscheck(sqvm, 3, ".sc");
  sq_rawset(sqvm, -3);

  sq_newarray(sqvm, 0);
  SqObjPtr refHolder;
  refHolder.attachToStack(sqvm, -1);
  sq_poptop(sqvm);

  sq_pushstring(sqvm, "keepref", 7);
  sq_pushobject(sqvm, refHolder.o);
  sq_newclosure(sqvm, keepref, 1);
  sq_rawset(sqvm, -3);


  sq_pushstring(sqvm, "__name__", 8);
  sq_pushstring(sqvm, "__main__", -1);
  sq_rawset(sqvm, -3);
  ...
  assert(sq_gettop(sqvm) == prevTop+1); // bindings table
  sq_poptop(sqvm); //bindings table
}


void SqModules::registerBaseLibs()
{
  registerNativeModule("math", sqstd_register_mathlib);
  registerNativeModule("string", sqstd_register_stringlib);
  registerNativeModule("blob", sqstd_register_bloblib);
}


void SqModules::registerSystemLib()
{
  registerNativeModule("system", sqstd_register_systemlib);
}

void SqModules::registerIoLib()
{
  registerNativeModule("io", sqstd_register_iolib);
}

void SqModules::registerDateTimeLib()
{
  registerNativeModule("datetime", sqstd_register_datetimelib);
}
