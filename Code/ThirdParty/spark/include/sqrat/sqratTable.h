...
public:
    ...
    TableBase& Bind(const SQChar* name, Object& object) {
    ...
    TableBase& Bind(const SQChar* name, const Object& object) {
        sq_pushobject(vm, GetObject());
        sq_pushstring(vm, name, -1);
        sq_pushobject(vm, object.GetObject());
        SQRAT_VERIFY(SQ_SUCCEEDED(sq_newslot(vm, -3, false)));
    ...
    template <bool raw>
    bool DeleteSlotImpl(SQInteger index) const {
        sq_pushobject(vm, obj);
        sq_pushinteger(vm, index);
        if (SQ_FAILED(raw ? sq_rawdeleteslot(vm, -2, false) : sq_deleteslot(vm, -2, false))) {
            sq_pop(vm, 1);
    ...
    bool DeleteSlot(SQInteger index) const {
        return DeleteSlotImpl<false>(index);
    }

    bool RawDeleteSlot(SQInteger index) const {
        return DeleteSlotImpl<true>(index);
    }
